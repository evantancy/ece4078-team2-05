...
class Robot:
    ...
    def derivative_drive(self, drive_meas):
        ...
        lin_vel, ang_vel = self.convert_wheel_speeds(
    ...
    def covariance_drive(self, drive_meas):
        ...
        Jac1 = np.array(
            [
                ...
                [
                    -self.wheels_scale / self.wheels_width,
                    self.wheels_scale / self.wheels_width,
                ],
            ]
        )
        ...
        if ang_vel == 0:
            ...
            Jac2[0, 1] = -1 / (ang_vel ** 2) * (
                np.sin(th2) - np.sin(th)
            ) + lin_vel / ang_vel * (dt * np.cos(th2))

            Jac2[1, 0] = -1 / ang_vel * (np.cos(th2) - np.cos(th))
            Jac2[1, 1] = 1 / (ang_vel ** 2) * (
                np.cos(th2) - np.cos(th)
